...
const AutoDiscoverySection = ({
  ...
  const generateCommand = () => {
    ...
    let scriptUrl = `https://raw.githubusercontent.com/ziqing2022/komari-agent/refs/heads/main/${scriptFile}`;
    if (enableGhproxy && ghproxy) {
    ...
    switch (selectedPlatform) {
      ...
      case "docker": {
        ...
        finalCommand =
          ...
          `ghcr.io/ziqing2022/komari-agent:latest ` +
...
function GenerateCommandButton({
  ...
  const generateCommand = () => {
    ...
    let scriptUrl =
      `https://raw.githubusercontent.com/ziqing2022/komari-agent/refs/heads/main/${scriptFile}`;
    if (enableGhproxy) {
      if (enableGhproxy && ghproxy) {
        scriptUrl = scriptUrl.slice(8); // 去掉 https://
        if (ghproxy.endsWith("/")) {
  ...
  return (
    <Drawer direction={isMobile ? "bottom" : "right"}>
      <DrawerTrigger asChild>
        <div className="h-8 flex items-center gap-2 hover:underline cursor-pointer font-bold text-base select-none">
          <span
          ...
          >
            {isOnline ? t("nodeCard.online", "在线") : t("nodeCard.offline", "离线")}
          </Badge>
        </div>
      </DrawerTrigger>
      <DrawerContent>
